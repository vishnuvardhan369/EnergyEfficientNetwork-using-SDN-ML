...
def parse_logs(file_path="finaldata.txt"):
    clients = []
    servers = []
...
def run_ml(df):
    df_ml = df.dropna(subset=["latency"])
    if df_ml.empty:
        print("No valid data to train ML model.")
        return

    X = df_ml[["etx", "hop_count", "energy", "degree"]]
    y = df_ml["latency"]
    ...
    print("\nML Feature Weights:")
    print(importance)
    importance.to_csv("ml_weights.csv", index=False)
    print("Weights saved to ml_weights.csv")

def main():
    clients_df, servers_df = parse_logs("finaldata.txt")
    if clients_df.empty:
